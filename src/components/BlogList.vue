...
import { sanityClient, ALL_ARTICLES_QUERY } from '../sanity'
...
onMounted(async () => {
  document.title = 'Ressources — WadoCoaching'
  const metaDesc = document.querySelector<HTMLMetaElement>('meta[name="description"]')
  if (metaDesc) metaDesc.content = 'Articles et ressources sur le coaching professionnel et le management par WadoCoaching.'

  try {
    articles.value = await sanityClient.fetch<ArticlePreview[]>(ALL_ARTICLES_QUERY)
  } catch {
    error.value = true
  } finally {
    loading.value = false
